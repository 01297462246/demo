...
function ExecutionTime(start, end)
{
    ...
    if (parseInt(h * 3600 + m * 60 + s) > parseInt(end)) {
        ...
        alert("Hết giờ! Điểm: " + scoreDiv.innerHTML);
        return;
    }
    //Xuất thời gian:
    if (s < 10) {
    ...
    timeToPlay = setTimeout(ExecutionTime, 1000, (start + 1), end);
}
function ExecuteGame()
{
    //Reset lại điểm:
    scoreDiv.innerHTML = 0;
    //Reset lại trạng thái circle:
    for(let i = 0; i < circles.length; i++)
    {
        circles[i].style.display = 'inline-block';
        circles[i].style.backgroundColor = 'rgba(255, 255, 255, 0.3)';
        circles[i].style.border = '1px solid #fff';
    }
    //Thay đổi vị trí các phần tử circle:
    pos = ChangePosition();
    //Tính thời gian:
    timeToPlay = setTimeout(ExecutionTime, 0, 0, 25);
    //Game play:
    game = setTimeout(GamePlay, 0);
}
